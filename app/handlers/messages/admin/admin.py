...
from vkbottle import Keyboard, KeyboardButtonColor, Text, BaseStateGroup, CtxStorage
...
async def waiting_for_promo(message):
    ...
    if len(parts) != 3:
        await message.answer("Неверный формат! Пожалуйста, введите три значения: промокод, ссылку и дата формата YYYY-MM-DD")
        return
    ...
    try:
        ...
        cursor.execute("INSERT INTO promo (promo_code, link, data) VALUES (%s, %s, %s);", (promo_code, promo_link, promo_data))
        connection.commit()
...
async def waiting_for_ivent(message):
    ...
    if len(parts) != 3:
        await message.answer("Неверный формат! Пожалуйста, введите три значения: название ивента, ссылку и дата формата YYYY-MM-DD")
        return
    ...
    try:
        ...
        # Добавление записи в таблицу
        cursor.execute("INSERT INTO ivent (ivent_name, link, data) VALUES (%s, %s, %s);", (ivent_name, ivent_link, ivent_data))
        connection.commit()
